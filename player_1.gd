...
func spawn_bullet(delta):
	...
	curr_b.initi(Vector2.from_angle(rotation - PI / 2), delta, 1)
	curr_b.rotate(rotation - PI / 2)

func _shoot(_delta):
	if Input.is_action_pressed("ShootP1") and canShoot:
		spawn_bullet((_delta))
		canShoot = false
	elif not canShoot:
...
func _physics_process(delta):
	...
	for i in get_slide_collision_count():
		...
		if (collider.name == "TileMap" || collider.collision_layer != 16):
			continue
